...
test_scenario(Config, Scenario) ->
  ...
  case bdd_utils:assert(Result) of
    ...
    _ ->
      io:format(
        "~n\t\t*** FAIL (Result: ~p)***~n\t\tSTEPS:~n\t\t\tGIVEN: ~p~n\t\t\tWHEN: ~p~n\t\t\tTHEN: ~p~n",
        [Result, GivenSteps, WhenSteps, ThenSteps]
      ),
      Name
  end.
...
step_run(Config, Input, Step) ->
  StepFiles =
    [
      list_to_atom(bdd_utils:config(Config, feature))
      | bdd_utils:config(Config, secondary_step_files)
    ],
...
step_run(_Config, _Input, Step, []) ->
  ...
  case StepType of
    step_given ->
      io:format(
        "MISSING STEP: \"step(_Config, _Global, {step_given, _N, ~p}) -> false;\"~n",
        [StepAction]
      );

    step_when ->
      io:format(
        "MISSING STEP: \"step(_Config, _Given, {step_when, _N, ~p}) -> false;\"~n",
        [StepAction]
      );

    step_then ->
      io:format(
        "MISSING STEP: \"step(_Config, _Result, {step_then, _N, ~p}) -> false;\"~n",
        [StepAction]
      );

    _ ->
      io:format(
        "MISSING STEP: \"step(_Config, _, {~p, _N, ~p}) -> false;\"~n",
        [StepType, StepAction]
      )
  end,
...
scenario_steps([H | T], N, Given, When, Then) ->
  ...
  case Step of
    ...
    {unknown, Mystery} ->
      io:format("\t\tWARNING: No prefix match for ~p~n", [Mystery]),
      scenario_steps(T, N + 1, Given, When, Then)
  end;
