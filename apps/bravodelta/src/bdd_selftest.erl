...
assert(Bools) -> assert(Bools, true).
...
assert_sets(Set) -> assert([Expected =:= Test || {_Name, Expected, Test} <- Set]).

assert_result(Test, Set) ->
  Result =
    case assert_sets(Set) of
      true ->
        io:format("\tPass \"~p\"~n", [Test]),
        true;

      false ->
        io:format("\t*** FAIL \"~p\"! results: ~p~n", [Test, Set]),
        Test
    end,
  Result.
...
test(all) ->
  ...
  case assert(Results) of
    true ->
      io:format("Completed, All Pass.~n"),
      pass;

    _ ->
      io:format("Completed, FAIL.~n"),
      [Failed || Failed <- Results, Failed =/= true]
  end;

test(Test) -> test(Test, source()).
...
test(assert, Source) ->
  Cases =
    [
      ...
      {true, assert_1true},
      {true, assert_mixed},
      {true, assert_truefirst},
      {true, assert_truelast},
      {false, assert_1false},
      {false, assert_false}
    ],
  Result =
    [{Test, Expected, bdd_utils:assert(get_data(Source, Test))} || {Expected, Test} <- Cases],
