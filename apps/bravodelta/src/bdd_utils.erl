...
html_search(Match, Results) -> html_search(Match, Results, true).
...
html_find_link(Match, Input) ->
  ...
  case re:run(Input, RE) of
    {match, S} ->
      % we only want the 3rd expression!
      [_, _, {Start, Length} | _] = S,
...
http_get(Config, Page) -> http_get(Config, Page, ok).
